...
`pipeline_status` map + score fields. The backend now also pushes a
**Server-Sent Events** nudge when a score or WhatsApp reply lands (`/events/ticket`
+ `/events/stream`), letting the UI refetch immediately; polling stays as the
fallback. The canonical frontend docs (kabil-frontend/docs/REALTIME.md) cover the
client side.
